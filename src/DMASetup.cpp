...
void DMASetup::SetupDMAModule(DMAInterface& dmaEngine, std::vector<int>& dbData,
  ...
  CalculateDMAStreamSetupData(inputStreamSetupData, MAX_CHUNK_SIZE,
  ...
  CalculateDMAStreamSetupData(outputStreamSetupData, MAX_CHUNK_SIZE,
                              MAX_DDR_BURST_SIZE, MAX_DDR_SIZE_PER_CYCLE,
                              dbData, recordSize);
  ...
  crossbarConfigurationFinder.FindInputCrossbarSetupData(
      ANY_CHUNK, ANY_POSITION, inputStreamSetupData);
  crossbarConfigurationFinder.FindOutputCrossbarSetupData(
      ANY_CHUNK, ANY_POSITION, outputStreamSetupData);
...
void DMASetup::SetUpDMACrossbars(DMASetupData& streamSetupData,
  ...
  for (size_t currentChunkIndex = 0;
       currentChunkIndex < streamSetupData.crossbarSetupData.size();
       ++currentChunkIndex) {
    if (streamSetupData.isInputStream) {
      for (int currentOffset = 0; currentOffset < 4; currentOffset++) {
        dmaEngine.setBufferToInterfaceChunk(
            streamSetupData.streamID, currentChunkIndex, currentOffset,
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .chunkData[3 + currentOffset * 4],
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .chunkData[2 + currentOffset * 4],
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .chunkData[1 + currentOffset * 4],
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .chunkData[0 + currentOffset * 4]);
        dmaEngine.setBufferToInterfaceSourcePosition(
            streamSetupData.streamID, currentChunkIndex, currentOffset,
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .positionData[3 + currentOffset * 4],
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .positionData[2 + currentOffset * 4],
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .positionData[1 + currentOffset * 4],
            streamSetupData.crossbarSetupData[currentChunkIndex]
                .positionData[0 + currentOffset * 4]);
      }
    } else {
      for (int currentOffset = 0; currentOffset < 4; currentOffset++) {
...
void DMASetup::SetUpDMAIOStreams(DMASetupData& streamSetupData,
  ...
  for (auto& chunkIDPair : streamSetupData.recordChunkIDs) {
    dmaEngine.setRecordChunkIDs(streamSetupData.streamID,
                                std::get<0>(chunkIDPair),
                                std::get<1>(chunkIDPair));
  }
}
...
void DMASetup::CalculateDMAStreamSetupData(DMASetupData& streamSetupData,
  ...
  streamSetupData.recordsPerDDRBurst =
      pow(2, (int)log2(recordsPerMaxBurstSize));
