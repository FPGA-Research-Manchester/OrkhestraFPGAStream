...
// in the future.
std::vector<QueryNode> GraphCreator::makeGraph(std::string input_def_filename) {
  ...
  for (auto const& [node_name, node_parameters] : data) {
    ...
    graph_nodes_map.insert(
        {node_name, QueryNode{std::get<std::vector<std::string>>(
  ...
  std::vector<QueryNode> graph_nodes;
