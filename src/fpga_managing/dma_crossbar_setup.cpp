...
void DMACrossbarSetup::FindOutputCrossbarSetupData(
  ...
  const int cycle_count =
      ...
      (steps_per_cycle * output_stream_setup_data.chunks_per_record);
  ...
  CalculateInterfaceToBufferSetupConfig(
      ...
      output_stream_setup_data.chunks_per_record);
...
void DMACrossbarSetup::FindInputCrossbarSetupData(
  ...
  const int cycle_count =
      query_acceleration_constants::kDatapathLength /
      (steps_per_cycle * input_stream_setup_data.chunks_per_record);
