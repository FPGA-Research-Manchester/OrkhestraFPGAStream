...
#include <cstdint>
/**
 * Interface class for accessing DDR memory where the table data is stored.
 */
class MemoryBlockInterface {
 public:
  ...
  /**
   * Get the virtual address for modifying the data in the memory.
   * @return Pointer to the start of the virtual address block.
   */
  virtual auto GetVirtualAddress() -> volatile uint32_t* = 0;
  /**
   * Get the physical address to configure the hardware with.
