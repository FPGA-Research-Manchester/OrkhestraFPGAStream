...
/**
 * DMA module which handles AXI bursts and sends data to the acceleration
 * modules.
 */
class DMA : public AccelerationModule, public DMAInterface {
 public:
  ...
  /**
   * Constructor to setup the DMA module configuration class.
  ...
  /**
   * Give input stream information to the input controller.
  ...
  /**
   * Read input controller parameters.
  ...
  /**
   * Set the starting address of an input stream in DDR.
  ...
  /**
   * Read input stream's starting address.
  ...
  /**
   * Find out if input controller has fully streamed all input streams.
   * @return Boolean showing if the input controller has finished.
   */
  auto IsInputControllerFinished() -> bool override;

  /**
   * Set record size in terms of integers of a stream.
  ...
  /**
   * Give chunk IDs to different chunks of a record.
   * @param stream_id ID of the configured stream.
  ...
  /**
   * Give input stream information to the output controller.
  ...
  void SetOutputControllerParams(int stream_id, int ddr_burst_size,
                                 ...
                                 int buffer_end) override;
  /**
   * Read output controller parameters.
  ...
  /**
   * Set the starting address of an output stream in DDR.
   * @param stream_id Output stream ID.
   * @param address Pointer to the starting point of the stream data.
   */
  void SetOutputControllerStreamAddress(int stream_id,
                                        uintptr_t address) override;
  /**
   * Read output stream's starting address.
   * @param stream_id Output stream ID.
   * @return Pointer to the start of the stream's data in DDR.
   */
  auto GetOutputControllerStreamAddress(int stream_id)
      -> volatile uintptr_t override;
  /**
   * Set how many records does the output stream have. For output streams we
   *  can't know that so it's for setting the number to 0.
   * @param stream_id Output stream ID.
  ...
  /**
   * Start the output controller to start pulling the streams through the
   *  modules and write the results back to the DDR. More specifically the output
   *  controller will start sending requrests of the final results which will get
   *  propagated through the infrastructure.
   * @param stream_active Array of booleans showing which streams should start
   *  getting pulled through the accelerators.
   */
  void StartOutputController(bool stream_active[16]) override;
  /**
   * Find out if output controller has fully streamed all output streams.
   * @return Boolean which says if all streams have been processed.
   */
  auto IsOutputControllerFinished() -> bool override;

  /**
   * Method to configure the input crossbar chunks. In-depth explanation is in
   *  the README.
  ...
  /**
   * Method to configure the input crossbar positions. In-depth explanation is
   *  in the README.
  ...
  /**
   * Method to configure the output crossbar chunks. In-depth explanation is in
   *  the README.
  ...
  /**
   * Method to configure the output crossbar positions. In-depth explanation is
   *  in the README.
   * @param stream_id Output stream ID.
   * @param clock_cycle Clock cycle selection.
   * @param offset Configuration 4 position offset selection.
  ...
  /**
   * Set the number of input streams which are going to have multiple channels.
   * @param number Multichannel stream count.
   */
  void SetNumberOfInputStreamsWithMultipleChannels(int number) override;
  /**
   * Set the number of records that will be streamed within a single burst for
  ...
  /**
   * Set the DDR burst length for the given multichannel stream.
  ...
  /**
   * Set the number of used channels for the given multichannel stream.
  ...
  /**
   * Set the starting address of a stream's channel.
   * @param stream_id Multichannel stream ID.
  ...
  void SetAddressForMultiChannelStreams(int stream_id, int channel_id,
                                        uintptr_t address) override;
  /**
   * Set how many records should be streamed into the given channel of the given
   *  stream.
   * @param stream_id Multichannel stream ID.
  ...
  /**
   * Debug method to get the runtime of the run. Measured by the hardware.
   * @return Amount of clock cycles spent on the execution of the query node.
   */
  auto GetRuntime() -> volatile uint64_t override;
  /**
   * Debug method to read the valid read cycle counter register.
   * @return Number of cycles there were valid reads.
   */
  auto GetValidReadCyclesCount() -> volatile uint64_t override;
  /**
   * Debug method to read the valid write cycle counter register.
   * @return Number of cycles there were valid writes.
   */
  auto GetValidWriteCyclesCount() -> volatile uint64_t override;

  /**
   * Method to write to the reset register to reset all configured modules
   *  including the DMA. The reset signal will be high for 8 clock cycles.
   */
