...
DMA::DMA(int* volatile ctrlAXIbaseAddress)
    : AccelerationModule(ctrlAXIbaseAddress, 0) {}
...
void DMA::setInputControllerStreamSize(
    int streamID, int size) {  // starting size of stream in amount of records
...
void DMA::setRecordSize(int streamID, int recordSize) {
  AccelerationModule::writeToModule(((1 << 17) + (1 << 8) + (streamID * 4)),
                                    recordSize - 1);
}
...
void DMA::setRecordChunkIDs(int streamID, int interfaceCycle, int chunkID) {
  AccelerationModule::writeToModule(
      ((1 << 17) + (1 << 13) + (streamID << 8) + (interfaceCycle << 2)),
      chunkID);
}
...
void DMA::setOutputControllerStreamAddress(int streamID, uintptr_t address) {
  AccelerationModule::writeToModule(((1 << 16) + (1 << 6) + (streamID * 4)),
                                    address >> 4);
}
...
void DMA::setOutputControllerStreamSize(
    int streamID, int size) {  // starting size of stream in amount of records
  AccelerationModule::writeToModule(((1 << 16) + (2 << 6) + (streamID * 4)),
                                    size);
}
auto DMA::getOutputControllerStreamSize(int streamID)
...
void DMA::setBufferToInterfaceChunk(int streamID, int clockCycle, int offset,
                                    int sourceChunk4, int sourceChunk3,
                                    int sourceChunk2, int sourceChunk1) {
  ...
  AccelerationModule::writeToModule(
      ((2 << 17) + (1 << 16) + (streamID << 12) + (clockCycle << 5) +
       (offset << 2)),
      ((sourceChunk4 << 24) + (sourceChunk3 << 16) + (sourceChunk2 << 8) +
       sourceChunk1));
}
void DMA::setBufferToInterfaceSourcePosition(int streamID, int clockCycle,
  ...
  position {offset*4} etc..*/
  AccelerationModule::writeToModule(
      ((2 << 17) + (streamID << 12) + (clockCycle << 5) + (offset << 2)),
      ((sourcePosition4 << 24) + (sourcePosition3 << 16) +
       (sourcePosition2 << 8) + sourcePosition1));
}

// Input Crossbar from AXI/DDR Input to Buffers
void DMA::setAXItoBufferChunk(int streamID, int clockCycle, int offset,
                              int targetChunk4, int targetChunk3,
                              int targetChunk2, int targetChunk1) {
  ...
  AccelerationModule::writeToModule(
      ((2 << 18) + (1 << 17) + (streamID << 13) + (clockCycle << 5) +
       (offset << 2)),
      ((targetChunk4 << 24) + (targetChunk3 << 16) + (targetChunk2 << 8) +
       targetChunk1));
}
void DMA::setAXItoBufferSourcePosition(int streamID, int clockCycle, int offset,
                                       int sourcePosition4, int sourcePosition3,
                                       int sourcePosition2,
                                       int sourcePosition1) {
  ...
  AccelerationModule::writeToModule(
      ((2 << 18) + (streamID << 13) + (clockCycle << 5) + (offset << 2)),
      ((sourcePosition4 << 24) + (sourcePosition3 << 16) +
       (sourcePosition2 << 8) + sourcePosition1));
}

// Output Crossbar from Interface to Buffers
void DMA::setInterfaceToBufferChunk(int streamID, int clockCycle, int offset,
                                    int targetChunk4, int targetChunk3,
                                    int targetChunk2, int targetChunk1) {
  /*When 32-bit data packets inside the {clockCycle} clock cycle of a record
  ...
  AccelerationModule::writeToModule(
      ((3 << 17) + (1 << 16) + (streamID << 12) + (clockCycle << 5) +
       (offset << 2)),
      ((targetChunk4 << 24) + (targetChunk3 << 16) + (targetChunk2 << 8) +
       targetChunk1));
}
void DMA::setInterfaceToBufferSourcePosition(int streamID, int clockCycle,
                                             int offset, int sourcePosition4,
                                             int sourcePosition3,
                                             int sourcePosition2,
                                             int sourcePosition1) {
  ...
  AccelerationModule::writeToModule(
      ((3 << 17) + (streamID << 12) + (clockCycle << 5) + (offset << 2)),
      ((sourcePosition4 << 24) + (sourcePosition3 << 16) +
       (sourcePosition2 << 8) + sourcePosition1));
}

// Output Crossbar from Buffers to AXI/DDR
void DMA::setBufferToAXIChunk(int streamID, int clockCycle, int offset,
                              int sourceChunk4, int sourceChunk3,
                              int sourceChunk2, int sourceChunk1) {
  ...
  AccelerationModule::writeToModule(
      ((3 << 18) + (1 << 17) + (streamID << 13) + (clockCycle << 5) +
       (offset << 2)),
      ((sourceChunk4 << 24) + (sourceChunk3 << 16) + (sourceChunk2 << 8) +
       sourceChunk1));
}
void DMA::setBufferToAXISourcePosition(int streamID, int clockCycle, int offset,
                                       int sourcePosition4, int sourcePosition3,
                                       int sourcePosition2,
                                       int sourcePosition1) {
  ...
  sourceChunk1 represents position {offset*4} etc..*/
  AccelerationModule::writeToModule(
      ((3 << 18) + (streamID << 13) + (clockCycle << 5) + (offset << 2)),
      ((sourcePosition4 << 24) + (sourcePosition3 << 16) +
       (sourcePosition2 << 8) + sourcePosition1));
}
