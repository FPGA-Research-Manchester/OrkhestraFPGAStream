...
TEST(FilterSetupTest, FilterStreamsSetting) {
  ...
  EXPECT_CALL(mockFilter,
              filterSetStreamIDs(inputStreamID, outputStreamID, outputStreamID))
      .Times(1);
...
TEST(FilterSetupTest, FilterModesSetting) {
  ...
  MockFilter mockFilter;
  EXPECT_CALL(mockFilter,
              filterSetMode(expectedRequestOnInvalidIfLast,
                            expectedForwardInvalidRecordFirstChunk,
                            expectedForwardFullInvalidRecords,
                            expectedFirstModuleInResourceElasticChain,
                            expectedLastModuleInResourceElasticChain))
      .Times(1);
  FilterSetup filterConfigurer;
  filterConfigurer.SetupFilterModule(mockFilter, inputStreamID, outputStreamID);
}
TEST(FilterSetupTest, CompareTypesSetting) {
  ...
  MockFilter mockFilter;
  EXPECT_CALL(mockFilter,
              filterSetCompareTypes(expectedChunkID, expectedPosition,
                                    expectedLessThanCompare, testing::_,
                                    testing::_, testing::_))
      .Times(1);
  FilterSetup filterConfigurer;
  filterConfigurer.SetupFilterModule(mockFilter, inputStreamID, outputStreamID);
...
TEST(FilterSetupTest, ReferenceValuesSetting) {
  ...
  MockFilter mockFilter;
  EXPECT_CALL(mockFilter,
              filterSetCompareReferenceValue(expectedChunkID, expectedPosition,
                                             expectedCompareUnitIndex,
                                             expectedCompareReferenceValue))
      .Times(1);
  FilterSetup filterConfigurer;
  filterConfigurer.SetupFilterModule(mockFilter, inputStreamID, outputStreamID);
}
TEST(FilterSetupTest, DNFClauseSetting) {
  ...
  MockFilter mockFilter;
  EXPECT_CALL(mockFilter, filterSetDNFClauseLiteral(
                              expectedDNFClauseID, expectedCompareUnitIndex,
                              expectedChunkID, expectedPosition,
                              expectedPositiveLiteralType))
      .Times(1);
  FilterSetup filterConfigurer;
  filterConfigurer.SetupFilterModule(mockFilter, inputStreamID, outputStreamID);
}
TEST(FilterSetupTest, CorrectFilterCalled) {
  ...
  MockFilter mockFilter;
  EXPECT_CALL(mockFilter,
              writeDNFClauseLiteralsToFilter_1CMP_8DNF(expectedDatapathWidth))
      .Times(1);
  EXPECT_CALL(mockFilter, writeDNFClauseLiteralsToFilter_2CMP_16DNF(testing::_))
      .Times(0);
  EXPECT_CALL(mockFilter, writeDNFClauseLiteralsToFilter_4CMP_32DNF(testing::_))
      .Times(0);
  FilterSetup filterConfigurer;
  filterConfigurer.SetupFilterModule(mockFilter, inputStreamID, outputStreamID);
}
