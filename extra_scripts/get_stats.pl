...
my @generator_query_count = (4,3,2);
my @selectivity = (0.75,0.5,0.25);
my @timeouts = (0.1,1,10);
my $hard_node_limit = 20;
                    ...
                    for my $join_c (@join_chance){
                        for my $arith_c (@arithmetic_chance){
                            for my $query_c (@generator_query_count){
                                ...
                                print $stats_file "\n$filter_c,$filter_dnf_low[$filter_size_i],$filter_comp_low[$filter_size_i],$filter_dnf_high[$filter_size_i],$filter_comp_high[$filter_size_i],$empty_j,$join_c,$arith_c,$arith_c,0.5,$query_c,$table_low[$table_size_i],$table_upper[$table_size_i],$hard_node_limit";
                                close $stats_file;
                                ...
                                for(my $j = 0; $j <= $#selectivity; $j++){
                                    for(my $i = 0; $i <= $#timeouts; $i++){
                                        open($stats_file, ">>$stats_filename");
                                        print $stats_file ",$selectivity[$j],$timeouts[$i],$equal_scaler,$equal_scaler,$preferred_scaler";
                                        close $stats_file;
                                        my $ret3 = system("python schedule.py $stats_filename $graph_filename $table_filename");
                                        if (!($i == $#timeouts and $j == $#selectivity)){
                                            open($stats_file, ">>$stats_filename");
                                            print $stats_file "\n$generated_query";
                                            close $stats_file;
                                        }
