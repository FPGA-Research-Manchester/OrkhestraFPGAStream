...
def count_node_operator(query_dicts, input_graph,
                        query_name, current_node_name):
    ...
    for table_name in input_graph[current_node_name]["input"]:
        if table_name != "":
            query_dicts[query_name]["table_names"].add(table_name)
    if input_graph[current_node_name]["operation_parameters"]["operation_params"]:
        if current_operation in query_dicts[query_name]["operation_capacities"].keys(
            ...
            query_dicts[query_name]["operation_capacities"][current_operation].append(
                [input_graph[current_node_name]["operation_parameters"]["operation_params"][4][0], input_graph[current_node_name]["operation_parameters"]["operation_params"][0][2]])
        else:
            query_dicts[query_name]["operation_capacities"][current_operation] = [
                [
                    input_graph[current_node_name]["operation_parameters"]["operation_params"][4][0], input_graph[current_node_name]["operation_parameters"]["operation_params"][0][2]]]
    for next_node_name in input_graph[current_node_name]["previous_nodes"]:
...
def main(argv):
    ...
    for node_name, node_parameters in input_graph.items():
        ...
        if len(node_parameters["next_nodes"]
               ) == 1 and node_parameters["next_nodes"][0] is None:
            end_node_names.add(node_name)

    stats_list = []
    operator_list = ["kFilter", "kLinearSort", "kMergeSort",
                     "kJoin", "kAddition", "kMultiplication", "kAggregationSum"]
    ...
    # Since all queries end with a single graph we first find all of these
    # finishing nodes and then start adding node names to the queries
    ...
    # Then I can go through all of the queries and note down all of their
    # corresponding nodes and collect query specific stats.
        ...
        if operation in global_operation_counters:
            ...
            for query_name in query_dicts.keys():
                ...
                if operation in query_dicts[query_name]["operation_capacities"]:
                    for operation_instance_i in range(
                            len(query_dicts[query_name]["operation_capacities"][operation])):
                        for capacity_parameter_i in range(len(
                                query_dicts[query_name]["operation_capacities"][operation][operation_instance_i])):
    ...
    table_sizes = []
    for table_parameters in input_tables:
