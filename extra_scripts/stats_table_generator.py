...
def get_stats(csvfile, y_keys, x_values, x_key, series_values, series_key, id_key, filter, filter_key, splitting_values, splitting_key):
    y_values = dict()
    for i in y_keys:
        y_values[i] = []
    series_stats = dict()
    ...
    if splitting_values:
        ...
        all_stats.append(copy.deepcopy(stats_in_range))
    read_rows(
        csvfile, x_key, series_key, y_values, all_stats, id_key, filter, filter_key, splitting_values, splitting_key)
...
def read_rows(csvfile, x_key, series_key, y_values, all_stats, id_key, filter, filter_key, splitting_values, splitting_key):
    ...
    for row in reader:
        if row[id_key] != last_id_value:
            if rowbuffer:
                if all(filter(rowbuffer, row, filter_key)):
...
def read_and_write_stats(stats_file_name, series_names, function_dict, y_keys, x_values, x_key, series_values, series_key, selected_functions, output_file_name, id_key, filter, filter_key, splitting_key="", splitting_values=[]):
    with open(stats_file_name, newline='') as csvfile:
        all_stats = get_stats(csvfile, y_keys, x_values,
                              x_key, series_values, series_key, id_key, filter, filter_key, splitting_values, splitting_key)
...
def main(argv):
    ...
    orig_stats_file_name = "stats40.csv"
    ...
    create_clean_stats(orig_stats_file_name, clean_stats_file_name,
                       rowbuffer, last_id_value, id_key, valid_key)
    ...
    id_key = "table_size_std_dev"
    def filter(rowbuffer, row, filter_key): return [True]
    filter_key = ""
    read_and_write_stats(clean_stats_file_name, series_names, function_dict, y_keys, x_values,
                         x_key, series_values, series_key, selected_functions, output_file_name, id_key, filter, filter_key)
    ...
    read_and_write_stats(clean_stats_file_name, series_names, function_dict, y_keys, x_values,
                         x_key, series_values, series_key, selected_functions, output_file_name, id_key, filter, filter_key, splitting_key, splitting_values)

    # Time, heuristic, timeout, utility, frames, utility_per_frames, table_size_mean - Equal timeouts
    # Avg, std_dev, count
    output_file_name = 'time_limits_split.csv'
    filter_key = "timeouts"
    def filter(rowbuffer, row, filter_key): return [
...
def create_clean_stats(orig_stats_file_name, clean_stats_file_name, rowbuffer, last_id_value, id_key, valid_key):
    with open(orig_stats_file_name, newline='') as orig_csvfile:
        ...
        with open(clean_stats_file_name, 'w', newline='') as clean_csvfile:
            ...
            for row in reader:
                if row[id_key] != last_id_value:
                    if rowbuffer:
                        if all(buffered_row[valid_key] for buffered_row in rowbuffer):
                            for buffered_row in rowbuffer:
